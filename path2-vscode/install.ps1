...
# ── Step 7: Fix skills with assets ─────────────────────────────────────────────────────────────
# After pac push, skills that use bic:bundle= references are broken in the target env.
# The bundle blob is in Azure file storage (env-specific, not in YAML or binary in clone).
# 
# FIX (automated): Read SKILL.md from the exported skills-with-assets/ folder,
# then PATCH the type-9 skill's data field to use inline InlineAgentSkill content.
# The skill instructions work. Python execution requires manual ZIP re-upload via UI.

if (Test-Path $SkillsDir) {
    $skillFolders = Get-ChildItem $SkillsDir -Directory -ErrorAction SilentlyContinue
    if ($skillFolders.Count -gt 0) {
        Step "Step 7 — Fix skills with assets (convert to inline InlineAgentSkill)"

        $allComps = (Invoke-RestMethod -Uri "$OrgNoTrail/api/data/v9.2/botcomponents?`=_parentbotid_value eq '$newBotId' and componenttype eq 9&`=botcomponentid,name,data" -Headers $dv).value
        $brokenSkills = $allComps | Where-Object { <#
.SYNOPSIS
    Deploy a Modern Copilot Studio agent from a VS Code clone to any target environment.
...
.data -like "*bic:bundle=*" }
        ...
        foreach ($sf in $skillFolders) {
            ...
            $skillMdText = Get-Content $skillMdPath -Raw
            # Find matching broken skill by name from SKILL.md
            $skillDisplayName = if ($skillMdText -match "(?m)^name:\s*(.+)") { $Matches[1].Trim() } else { $sf.Name }
            $broken = $brokenSkills | Where-Object { <#
.SYNOPSIS
    Deploy a Modern Copilot Studio agent from a VS Code clone to any target environment.
...
.name -eq $sf.Name }
            ...
            if (-not $broken) { WARN "No broken skill matching '$skillDisplayName' found — skipping"; continue }

            # Build inline InlineAgentSkill
...
" }
            ...
            Invoke-RestMethod -Uri "$OrgNoTrail/api/data/v9.2/botcomponents($($broken.botcomponentid))" -Method PATCH -Headers $dv -Body (@{ data = $newData } | ConvertTo-Json) | Out-Null
            OK "  '$skillDisplayName' fixed: bic:bundle= → inline InlineAgentSkill"
            WARN "  Python execution unavailable — re-upload ZIP via UI to restore"
        }
    }
}
