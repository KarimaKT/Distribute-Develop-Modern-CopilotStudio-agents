...
# A) Automated inline fix: reads SKILL.md from skills-with-assets/ and patches the
#    type-9 skill data to inline InlineAgentSkill. Agent works immediately.
# B) Guided re-upload: rebuilds ZIP, opens browser, prints exact steps.
...
if (Test-Path $SkillsDir) {
    ...
    if ($skillFolders.Count -gt 0) {
        ...
        foreach ($sf in $skillFolders) {
            ...
            if (-not $broken) { WARN "No broken skill matching '$dispName' — skipping"; continue }
        ...
        if ($reuploadList.Count -gt 0) {
            ...
            Write-Host ""
            Write-Host "  OPTIONAL: Restore Python execution (agent already works without this):" -ForegroundColor Cyan
            foreach ($r in $reuploadList) {
                Write-Host "    • $($r.name)  →  ZIP: $($r.zipPath)" -ForegroundColor Cyan
            }
            Write-Host "    1. Open: $agentUrl" -ForegroundColor Cyan
            Write-Host "    2. Skills → × remove the skill → Add skill → Upload → select ZIP above → Save"
