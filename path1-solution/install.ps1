...
# After solution import, skills uploaded as ZIP files have a stale bic:bundle= reference.
# The bundle blob is stored in Azure file storage outside DV's component graph and is
# NOT included in the solution package. The type-14 file components (SKILL.md, Python scripts)
# ARE included and contain the binary content.
#
# FIX: Read SKILL.md from the imported type-14 file component, then PATCH the type-9
# skill's data field to replace the stale bic:bundle= with inline InlineAgentSkill content.
#
# LIMITATION: This converts the skill from a bundle-backed skill to an inline skill.
# The skill INSTRUCTIONS work correctly. The Python execution capability is lost.
# For skills where Python execution is required, the user must manually re-upload the
# original ZIP via Copilot Studio UI → Skills → Upload a skill.
#
# WHAT IS RECOVERED AUTOMATICALLY:
#   - Skill name and description
#   - Full SKILL.md instructions (what the agent reads to use the skill)
#   - Skill is usable and appears in the agent Build view
#
# WHAT REQUIRES MANUAL RE-UPLOAD (if Python execution needed):
#   - Binary bundle that allows Code Interpreter to execute the Python script at runtime

if (.skillsWithAssets.Count -gt 0) {
    Step "Step 2 — Fix skills with assets (bic:bundle= → inline InlineAgentSkill)"

    # Find the imported bot
    $importedBot = (Invoke-RestMethod -Uri "$OrgNoTrail/api/data/v9.2/bots?`=schemaname eq '$($manifest.agentSchema)'&`=botid" -Headers $dv).value[0]
    $importedBotId = $importedBot.botid
    INFO "Bot: $importedBotId"

    # Find all broken skills (bic:bundle= reference in data field)
    $allImportedComps = (Invoke-RestMethod -Uri "$OrgNoTrail/api/data/v9.2/botcomponents?`=_parentbotid_value eq '$importedBotId' and componenttype eq 9&`=botcomponentid,name,data" -Headers $dv).value
    $brokenSkills = $allImportedComps | Where-Object { <#
.SYNOPSIS
    Import a Modern Copilot Studio agent from an export bundle (agent.zip + skills-with-assets/).
